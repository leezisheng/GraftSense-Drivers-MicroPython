...
# @Author  : stewedio
# @File    : util.py
# @Description : ATECC608A/ATECC508A config zone dump utilities (slot config, key config, full configuration decoding)
# @License : MIT
...
# SlotConfig 位域结构定义（每个 slot 2 字节，共 16 slots）
# SlotConfig bitfield structure definition (2 bytes per slot, 16 slots total)
SLOT_CONFIG_STRUCT = {
...
# 解析并打印单个 Slot 配置（2 字节）
# Parse and print a single Slot config (2 bytes)
def dump_slot(slot, index=None, stream=None):
    slot_stuct = uctypes.struct(uctypes.addressof(slot), SLOT_CONFIG_STRUCT, uctypes.LITTLE_ENDIAN)
...
# KeyConfig 位域结构定义（每个 key 2 字节，共 16 keys）
# KeyConfig bitfield structure definition (2 bytes per key, 16 keys total)
KEY_CONFIG_STRUCT = {
...
# 解析并打印单个 Key 配置（2 字节）
# Parse and print a single Key config (2 bytes)
def dump_key(key, index=None, stream=None):
    key_stuct = uctypes.struct(uctypes.addressof(key), KEY_CONFIG_STRUCT, uctypes.LITTLE_ENDIAN)
...
# 解析并打印完整的 128 字节配置区数据
# Parse and print the complete 128-byte configuration zone
