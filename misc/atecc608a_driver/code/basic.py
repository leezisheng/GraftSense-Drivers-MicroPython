...
# @Author  : stewedio
# @File    : basic.py
# @Description : ATECC basic API methods - provides all CryptoAuthLib command wrappers (CheckMAC, Counter, DeriveKey, ECDH, GenDig, GenKey, Info, Lock, Nonce, Random, Read, SHA, Sign, Verify, Write, etc.)
# @License : MIT

import constant as ATCA_CONSTANTS
import exceptions as ATCA_EXCEPTIONS
import status as ATCA_STATUS
from packet import ATCAPacket
...
class ATECCBasic(object):
    ...
    def is_error(self, data):
        ...
        if data[ATCA_CONSTANTS.ATCA_COUNT_IDX] == 0x04:
            return ATCA_STATUS.decode_error(data[ATCA_CONSTANTS.ATCA_RSP_DATA_IDX])
        else:
    ...
    def atcab_get_addr(self, zone, slot=0, block=0, offset=0):
        # 获取存储区地址
        # Get zone address from zone, slot, block, and offset
        mem_zone = zone & ATCA_CONSTANTS.ATCA_ZONE_MASK
        if mem_zone not in (ATCA_CONSTANTS.ATCA_ZONE_CONFIG, ATCA_CONSTANTS.ATCA_ZONE_DATA, ATCA_CONSTANTS.ATCA_ZONE_OTP):
        ...
        addr = 0
        offset = offset & 0x07
        if mem_zone in (ATCA_CONSTANTS.ATCA_ZONE_CONFIG, ATCA_CONSTANTS.ATCA_ZONE_OTP):
            addr = block << 3
            addr = addr | offset
    ...
    def atcab_get_zone_size(self, zone, slot=0):
        # 获取存储区大小
        # Get zone size
        if zone not in (ATCA_CONSTANTS.ATCA_ZONE_CONFIG, ATCA_CONSTANTS.ATCA_ZONE_DATA, ATCA_CONSTANTS.ATCA_ZONE_OTP):
            raise ATCA_EXCEPTIONS.BadArgumentError()
    ...
    def atcab_gendig(self, zone, key_id, other_data):
        ...
        if zone < 0x00 or zone > 0x05:
            raise ATCA_EXCEPTIONS.BadArgumentError("Zone must be between zero and 5")

        packet = ATCAPacket(opcode=ATCA_CONSTANTS.ATCA_GENDIG, param1=zone, param2=key_id, request_data=other_data)
        self.execute(packet)
        return packet
    ...
    def atcab_genkey_base(self, mode, key_id, other_data=None):
        ...
        packet = ATCAPacket(txsize=txsize, opcode=ATCA_CONSTANTS.ATCA_GENKEY, param1=mode, param2=key_id, request_data=data)
        self.execute(packet)
        return packet
    ...
    def atcab_info_base(self, mode=0):
        packet = ATCAPacket(opcode=ATCA_CONSTANTS.ATCA_INFO, param1=mode)
        self.execute(packet)
        return packet
    ...
    def atcab_lock(self, mode, crc=0):
        packet = ATCAPacket(txsize=ATCA_CONSTANTS.LOCK_COUNT, opcode=ATCA_CONSTANTS.ATCA_LOCK, param1=mode, param2=crc)
        self.execute(packet)
        return packet

    def atcab_lock_config_zone(self):
        return self.atcab_lock(ATCA_CONSTANTS.LOCK_ZONE_NO_CRC | ATCA_CONSTANTS.LOCK_ZONE_CONFIG)
    ...
    def atcab_lock_data_zone(self):
        return self.atcab_lock(ATCA_CONSTANTS.LOCK_ZONE_NO_CRC | ATCA_CONSTANTS.LOCK_ZONE_DATA)
    ...
    def atcab_nonce_base(self, mode, zero=0, numbers=None):
        ...
        if nonce_mode not in (ATCA_CONSTANTS.NONCE_MODE_SEED_UPDATE, ATCA_CONSTANTS.NONCE_MODE_NO_SEED_UPDATE, ATCA_CONSTANTS.NONCE_MODE_PASSTHROUGH):
            raise ATCA_EXCEPTIONS.BadArgumentError()
        ...
        txsize = 0
        if nonce_mode in (ATCA_CONSTANTS.NONCE_MODE_SEED_UPDATE, ATCA_CONSTANTS.NONCE_MODE_NO_SEED_UPDATE):
        ...
        packet = ATCAPacket(
            txsize=txsize, opcode=ATCA_CONSTANTS.ATCA_NONCE, param1=mode, param2=zero, request_data=n_mv[: txsize - ATCA_CONSTANTS.ATCA_CMD_SIZE_MIN]
        )
    ...
    def atcab_nonce(self, numbers=None):
        return self.atcab_nonce_base(ATCA_CONSTANTS.NONCE_MODE_PASSTHROUGH, numbers=numbers)
    ...
    def atcab_nonce_rand(self, numbers=None):
        return self.atcab_nonce_base(ATCA_CONSTANTS.NONCE_MODE_SEED_UPDATE, numbers=numbers)
    ...
    def atcab_read_zone(self, zone, slot=0, block=0, offset=0, length=0):
        # 校验读取长度（仅支持 4 字节和 32 字节）
        if length not in (ATCA_CONSTANTS.ATCA_WORD_SIZE, ATCA_CONSTANTS.ATCA_BLOCK_SIZE):
            raise ATCA_EXCEPTIONS.BadArgumentError()
        ...
        packet = ATCAPacket(opcode=ATCA_CONSTANTS.ATCA_READ, param1=zone, param2=addr)
        self.execute(packet)
        return packet

    def atcab_read_serial_number(self):
        # 读取芯片序列号（config zone 的前 32 字节）
        return self.atcab_read_zone(ATCA_CONSTANTS.ATCA_ZONE_CONFIG, length=ATCA_CONSTANTS.ATCA_BLOCK_SIZE)
    ...
    def atcab_read_bytes_zone(self, zone, slot=0, block=0, offset=0, length=0):
        ...
        while d_idx < length:
            ...
            packet = self.atcab_read_zone(zone, slot=slot, block=c_blk, offset=c_of, length=r_sz)
    ...
    def atcab_is_locked(self, zone):
        # 校验 zone 参数
        if zone not in (ATCA_CONSTANTS.LOCK_ZONE_CONFIG, ATCA_CONSTANTS.LOCK_ZONE_DATA):
            raise ATCA_EXCEPTIONS.BadArgumentError()

        # 读取 LockValue/LockConfig 域（config block=2, word offset=5）
        # Read UserExtra, Selector, LockValue, LockConfig
        packet = self.atcab_read_zone(ATCA_CONSTANTS.ATCA_ZONE_CONFIG, slot=0, block=2, offset=5, length=ATCA_CONSTANTS.ATCA_WORD_SIZE)
    ...
    def atcab_read_config_zone(self):
        # 读取完整配置区（128 字节）
        return self.atcab_read_bytes_zone(ATCA_CONSTANTS.ATCA_ZONE_CONFIG, length=ATCA_CONSTANTS.ATCA_ECC_CONFIG_SIZE)
    ...
    def atcab_read_enc(self, key_id, block, enc_key_id, num_in=b"\x00" * 32):
        ...
        # 将 Nonce 直通写入 TempKey（p71）
        # Write nonce to TempKey in pass through mode
        self.atcab_nonce(num_in)

        # 构造 OtherData 确保 GenDig 行为一致（NoMac set）
        ...
        # 生成摘要并从 TempKey.value 获取会话密钥（p68）
        # Generate digest and retrieve session key from TempKey.value
        gendig_packet = self.atcab_gendig(ATCA_CONSTANTS.GENDIG_ZONE_DATA, enc_key_id, other_data)
        session_key = gendig_packet.response_data[-32:]

        # 读取密文（p81）
        # Read cipher text value
        read_packet = self.atcab_read_zone(
            ATCA_CONSTANTS.ATCA_ZONE_DATA | ATCA_CONSTANTS.ATCA_ZONE_READWRITE_32, key_id, block, 0, ATCA_CONSTANTS.ATCA_BLOCK_SIZE
        )
    ...
    def atcab_selftest(self, mode, param2=0):
        ...
        packet = ATCAPacket(opcode=ATCA_CONSTANTS.ATCA_SELFTEST, param1=mode, param2=param2)
        self.execute(packet)
    ...
    def atcab_sha_base(self, mode=0, data=b"", key_slot=None):
        ...
        if cmd_mode in (ATCA_CONSTANTS.SHA_MODE_SHA256_START, ATCA_CONSTANTS.SHA_MODE_HMAC_START, ATCA_CONSTANTS.SHA_MODE_SHA256_PUBLIC):
            txsize = ATCA_CONSTANTS.ATCA_CMD_SIZE_MIN
        elif cmd_mode in (ATCA_CONSTANTS.SHA_MODE_SHA256_UPDATE, ATCA_CONSTANTS.SHA_MODE_SHA256_END, ATCA_CONSTANTS.SHA_MODE_HMAC_END):
    ...
    def atcab_sign_base(self, mode, key_id):
        packet = ATCAPacket(txsize=ATCA_CONSTANTS.SIGN_COUNT, opcode=ATCA_CONSTANTS.ATCA_SIGN, param1=mode, param2=key_id)
        self.execute(packet)
        return packet
    ...
    def atcab_sign(self, key_id, message):
        ...
        return self.atcab_sign_base(ATCA_CONSTANTS.SIGN_MODE_EXTERNAL | sign_source, key_id)
    ...
    def atcab_updateextra(self, mode, value):
        packet = ATCAPacket(opcode=ATCA_CONSTANTS.ATCA_UPDATE_EXTRA, param1=mode, param2=value)
        self.execute(packet)
        return packet
    ...
    def atcab_verify(self, mode, key_id, signature, public_key=None, other_data=None, mac=None):
        ...
        elif verify_mode in (ATCA_CONSTANTS.VERIFY_MODE_VALIDATE_EXTERNAL, ATCA_CONSTANTS.VERIFY_MODE_EXTERNAL):
            txsize = ATCA_CONSTANTS.VERIFY_256_EXTERNAL_COUNT
        elif verify_mode in (ATCA_CONSTANTS.VERIFY_MODE_VALIDATE, ATCA_CONSTANTS.VERIFY_MODE_INVALIDATE):
        ...
        packet = ATCAPacket(txsize=txsize, opcode=ATCA_CONSTANTS.ATCA_VERIFY, param1=mode, param2=key_id, request_data=data)
        self.execute(packet)
        return packet
    ...
    def atcab_verify_extern(self, message, signature, public_key):
        ...
        self.atcab_nonce_load(nonce_target, message)
        try:
            packet = self.atcab_verify(ATCA_CONSTANTS.VERIFY_MODE_EXTERNAL | verify_source, ATCA_CONSTANTS.VERIFY_KEY_P256, signature, public_key)
    ...
    def atcab_verify_stored(self, message, signature, key_id):
        ...
        self.atcab_nonce_load(nonce_target, message)
        try:
            packet = self.atcab_verify(ATCA_CONSTANTS.VERIFY_MODE_STORED | verify_source, key_id, signature)
            return packet[1] == ATCA_STATUS.ATCA_SUCCESS
        except ATCA_EXCEPTIONS.CheckmacVerifyFailedError:
            # 验证失败但指令执行成功
            # Verify failed, but command succeeded
    ...
    def atcab_write_bytes_zone(self, zone, slot=0, offset=0, data=None):
        ...
        d_mv = memoryview(data)
        while d_idx < length:
            # 最后一个条目确保正确处理 config 区的 selector/user extra/lock 字节
            ...
            if c_wrd == 0 and length - d_idx >= BS and not (zone == ZC and c_blk == 2):
                packet = self.atcab_write_zone(zone, slot=slot, block=c_blk, offset=0, data=d_mv[d_idx : d_idx + BS])
                packets.append(packet)
                d_idx += BS
                c_blk += 1
            else:
                # 跳过 UserExtra/Selector/LockValue/LockConfig（需 UpdateExtra 指令修改）
                ...
                if not (zone == ZC and c_blk == 2 and c_wrd == 5):
                    packet = self.atcab_write_zone(zone, slot=slot, block=c_blk, offset=c_wrd, data=d_mv[d_idx : d_idx + WS])
                    packets.append(packet)
                d_idx += WS
                c_wrd += 1
                if c_wrd == BS // WS:
    ...
    def atcab_write_pubkey(self, slot, public_key):
        ...
        packets = self.atcab_write_bytes_zone(ATCA_CONSTANTS.ATCA_ZONE_DATA, slot=slot, data=public_key_formatted)
    ...
    def atcab_write_config_zone(self, config_data):
        ...
        # 写入 config zone（排除 UserExtra 和 Selector 字节）
        # Write config zone excluding UserExtra and Selector
        packets = self.atcab_write_bytes_zone(ATCA_CONSTANTS.ATCA_ZONE_CONFIG, slot=0, offset=16, data=config_data[16 : config_size - 16])

        # 写入 UserExtra（已有非零值则可能失败）
        # Write the UserExtra and Selector. This may fail if either value is already non-zero.
        packet = self.atcab_updateextra(ATCA_CONSTANTS.UPDATE_MODE_USER_EXTRA, config_data[84])
        packets.append(packet)

        packet = self.atcab_updateextra(ATCA_CONSTANTS.UPDATE_MODE_SELECTOR, config_data[85])
        packets.append(packet)
