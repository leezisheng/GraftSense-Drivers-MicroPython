...
# @File    : axp2101.py
# @Description : AXP2101 电源管理IC (PMU) MicroPython I2C 驱动，支持 DCDC/LDO 控制、ADC 采集、充电管理和中断处理
# @License : MIT

__version__ = "1.0.0"
__author__ = "Lewis He (lewishe@outlook.com)"
__license__ = "MIT"
...
def _to_bin(value: int, num: int) -> str:
    """
    将整数值转换为指定位宽的二进制字符串
    用于调试输出
    ==========================================
...
class AXP2101(I2CInterface):
    """
    AXP2101 电源管理IC (PMU) 驱动类
    Attributes:
        _address (int): I2C 从设备地址
        _debug (bool): 调试日志开关
        statusRegister (list): 中断状态寄存器缓存
        intRegister (list): 中断使能寄存器缓存
    Methods:
        getChipID(): 读取芯片 ID
        isBatteryConnect(): 检测电池是否连接
        getBattVoltage(): 读取电池电压
        getBatteryPercent(): 读取电池电量百分比
        enableDC1()/disableDC1(): DCDC1 开关控制
        setDC1Voltage(mv): 设置 DCDC1 输出电压
        enableALDO1()/disableALDO1(): ALDO1 开关控制
        setChargerConstantCurr(opt): 设置充电恒流电流
        setChargeTargetVoltage(opt): 设置充电目标电压
        enableIRQ(opt)/disableIRQ(opt): 中断使能控制
        getIrqStatus(): 读取中断状态
        shutdown(): 关闭所有电源通道
        deinit(): 释放硬件资源
    Notes:
        - 依赖外部传入 I2C 总线实例
        - 继承自 I2CInterface 获取底层寄存器读写能力
        - 默认 I2C 地址：0x34
        - 芯片 ID：0x4A
        - 涉及电源控制操作（shutdown/reset），在真实硬件上需谨慎使用
    ==========================================
    AXP2101 Power Management IC (PMU) driver class.
    Attributes:
        _address (int): I2C slave device address
        _debug (bool): Debug log switch
        statusRegister (list): Interrupt status register cache
        intRegister (list): Interrupt enable register cache
    Methods:
        getChipID(): Read chip ID
        isBatteryConnect(): Check battery connection status
        getBattVoltage(): Read battery voltage
        getBatteryPercent(): Read battery level percentage
        enableDC1()/disableDC1(): DCDC1 on/off control
        setDC1Voltage(mv): Set DCDC1 output voltage
        enableALDO1()/disableALDO1(): ALDO1 on/off control
        setChargerConstantCurr(opt): Set charging constant current
        setChargeTargetVoltage(opt): Set charging target voltage
        enableIRQ(opt)/disableIRQ(opt): Interrupt enable control
        getIrqStatus(): Read interrupt status
        shutdown(): Power off all channels
        deinit(): Release hardware resources
    Notes:
        - Requires externally provided I2C bus instance
        - Inherits from I2CInterface for low-level register access
        - Default I2C address: 0x34
        - Chip ID: 0x4A
        - Power control operations (shutdown/reset) must be used carefully on real hardware
    """

    # PMU 中断触发定时控制值 / PMU IRQ timing control values
    ...
    def __init__(self, i2c_bus, addr: int = AXP2101_SLAVE_ADDRESS, debug: bool = False) -> None:
        """
        初始化 AXP2101 PMU 驱动实例
        Args:
            i2c_bus (I2C): I2C 总线实例
            addr (int): I2C 从设备地址，默认 0x34
            debug (bool): 是否启用调试日志输出，默认 False
        Returns:
            None
        Raises:
            ValueError: 参数类型无效
            RuntimeError: 芯片 ID 验证失败，设备未正确连接
        Notes:
            - 初始化时自动验证芯片 ID
            - 不执行任何电源通道配置，由调用方按需设置
        ==========================================
        Initialize AXP2101 PMU driver instance.
        Args:
            i2c_bus (I2C): I2C bus instance
            addr (int): I2C slave address, default 0x34
            debug (bool): Enable debug log output, default False
        Returns:
            None
        Raises:
            ValueError: Invalid parameter type
            RuntimeError: Chip ID verification failed, device not properly connected
        Notes:
            - Automatically verifies chip ID during initialization
            - Does not configure any power channels; caller sets as needed
        """
        ...
        if chip_id != XPOWERS_AXP2101_CHIP_ID:
            raise RuntimeError("Failed to find AXP2101 (chip ID 0x%02X, expected 0x%02X) - check your wiring!" % (chip_id, XPOWERS_AXP2101_CHIP_ID))
    ...
    def setButtonBatteryChargeVoltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_BTN_VOL_STEPS:
            raise ValueError("Button battery charge step voltage must be %u mV" % _AXP2101_BTN_VOL_STEPS)
        if millivolt < _AXP2101_BTN_VOL_MIN:
            raise ValueError("The minimum charge termination voltage of the coin cell battery is %u mV" % _AXP2101_BTN_VOL_MIN)
        elif millivolt > _AXP2101_BTN_VOL_MAX:
            raise ValueError("The minimum charge termination voltage of the coin cell battery is %u mV" % _AXP2101_BTN_VOL_MAX)
    ...
    def setWatchdogConfig(self, opt: int) -> None:
        if not 0 <= opt <= 3:
            raise ValueError("Watchdog Config optrion must be a value within 0-3!")
    ...
    def setWatchdogTimeout(self, opt: int) -> None:
        if not 0 < opt <= 7:
            raise ValueError("Watchdog timeout must be a value within 0-7!")
        val = super().readRegister(_AXP2101_WDT_CTRL)[0]
        val &= 0xF8
    ...
    def setSysPowerDownVoltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_VSYS_VOL_THRESHOLD_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_VSYS_VOL_THRESHOLD_STEPS)
        if millivolt < _AXP2101_VSYS_VOL_THRESHOLD_MIN:
            raise ValueError("The minimum settable voltage of VSYS is %u mV" % _AXP2101_VSYS_VOL_THRESHOLD_MIN)
        elif millivolt > _AXP2101_VSYS_VOL_THRESHOLD_MAX:
            raise ValueError("The maximum settable voltage of VSYS is %u mV" % _AXP2101_VSYS_VOL_THRESHOLD_MAX)

        val = super().readRegister(_AXP2101_VOFF_SET)[0]
        val &= 0xF8
        super().writeRegister(_AXP2101_VOFF_SET, val | (int)((millivolt - _AXP2101_VSYS_VOL_THRESHOLD_MIN) / _AXP2101_VSYS_VOL_THRESHOLD_STEPS))
    ...
    def setDCFreqSpreadRange(self, opt: int) -> None:
        if not 0 <= opt <= 1:
            raise ValueError("set DCDC freq spread range must be a value within 0-1!")
        if opt:
    ...
    def setDC1Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_DCDC1_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_DCDC1_VOL_STEPS)
        if millivolt < _AXP2101_DCDC1_VOL_MIN:
            raise ValueError("DC1 minimum voltage is %u mV" % _AXP2101_DCDC1_VOL_MIN)
        elif millivolt > _AXP2101_DCDC1_VOL_MAX:
            raise ValueError("DC1 maximum voltage is %u mV" % _AXP2101_DCDC1_VOL_MAX)
        super().writeRegister(_AXP2101_DC_VOL0_CTRL, (int)((millivolt - _AXP2101_DCDC1_VOL_MIN) / _AXP2101_DCDC1_VOL_STEPS))
    ...
    def setDC2Voltage(self, millivolt: int) -> None:
        ...
        if millivolt >= _AXP2101_DCDC2_VOL1_MIN and millivolt <= _AXP2101_DCDC2_VOL1_MAX:
            if millivolt % _AXP2101_DCDC2_VOL_STEPS1:
                raise ValueError(" The steps is must %umV" % _AXP2101_DCDC2_VOL_STEPS1)
            super().writeRegister(_AXP2101_DC_VOL1_CTRL, val | (int)((millivolt - _AXP2101_DCDC2_VOL1_MIN) / _AXP2101_DCDC2_VOL_STEPS1))
        elif millivolt >= _AXP2101_DCDC2_VOL2_MIN and millivolt <= _AXP2101_DCDC2_VOL2_MAX:
            if millivolt % _AXP2101_DCDC2_VOL_STEPS2:
                raise ValueError(" The steps is must %umV" % _AXP2101_DCDC2_VOL_STEPS2)
            val |= (int)((((millivolt - _AXP2101_DCDC2_VOL2_MIN) / _AXP2101_DCDC2_VOL_STEPS2) + _AXP2101_DCDC2_VOL_STEPS2_BASE))
    ...
    def getDC2WorkMode(self) -> int:
        return super().getRegisterBit(_AXP2101_DC_VOL1_CTRL, 7)
    ...
    def setDC3Voltage(self, millivolt: int) -> None:
        ...
        if millivolt >= _AXP2101_DCDC3_VOL1_MIN and millivolt <= _AXP2101_DCDC3_VOL1_MAX:
            if millivolt % _AXP2101_DCDC3_VOL_STEPS1:
                raise ValueError("Step size must be %umV" % _AXP2101_DCDC3_VOL_STEPS1)
            super().writeRegister(_AXP2101_DC_VOL2_CTRL, val | (int)((millivolt - _AXP2101_DCDC3_VOL_MIN) / _AXP2101_DCDC3_VOL_STEPS1))
        elif millivolt >= _AXP2101_DCDC3_VOL2_MIN and millivolt <= _AXP2101_DCDC3_VOL2_MAX:
            if millivolt % _AXP2101_DCDC3_VOL_STEPS2:
                raise ValueError("Step size must be %umV" % _AXP2101_DCDC3_VOL_STEPS2)
            val |= (int)(((millivolt - _AXP2101_DCDC3_VOL2_MIN) / _AXP2101_DCDC3_VOL_STEPS2) + _AXP2101_DCDC3_VOL_STEPS2_BASE)
            ...
            if millivolt % _AXP2101_DCDC3_VOL_STEPS3:
                raise ValueError("Step size must be %umV" % _AXP2101_DCDC3_VOL_STEPS3)
            val |= (int)(((millivolt - _AXP2101_DCDC3_VOL3_MIN) / _AXP2101_DCDC3_VOL_STEPS3) + _AXP2101_DCDC3_VOL_STEPS3_BASE)
            super().writeRegister(_AXP2101_DC_VOL2_CTRL, val)
    ...
    def setDC4Voltage(self, millivolt: int) -> None:
        ...
        if millivolt >= _AXP2101_DCDC4_VOL1_MIN and millivolt <= _AXP2101_DCDC4_VOL1_MAX:
            if millivolt % _AXP2101_DCDC4_VOL_STEPS1:
                raise ValueError("Step size must be %umV" % _AXP2101_DCDC4_VOL_STEPS1)
            super().writeRegister(_AXP2101_DC_VOL3_CTRL, val | (int)((millivolt - _AXP2101_DCDC4_VOL1_MIN) / _AXP2101_DCDC4_VOL_STEPS1))
        elif millivolt >= _AXP2101_DCDC4_VOL2_MIN and millivolt <= _AXP2101_DCDC4_VOL2_MAX:
            if millivolt % _AXP2101_DCDC4_VOL_STEPS2:
                raise ValueError("Step size must be %umV" % _AXP2101_DCDC4_VOL_STEPS2)
            val |= (int)(((millivolt - _AXP2101_DCDC4_VOL2_MIN) / _AXP2101_DCDC4_VOL_STEPS2) + _AXP2101_DCDC4_VOL_STEPS2_BASE)
    ...
    def setDC5Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_DCDC5_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_DCDC5_VOL_STEPS)
        if millivolt != _AXP2101_DCDC5_VOL_1200MV and millivolt < _AXP2101_DCDC5_VOL_MIN:
            raise ValueError("DC5 minimum voltage is %umV ,%umV" % (_AXP2101_DCDC5_VOL_1200MV, _AXP2101_DCDC5_VOL_MIN))
        elif millivolt > _AXP2101_DCDC5_VOL_MAX:
            raise ValueError("DC5 maximum voltage is %umV" % _AXP2101_DCDC5_VOL_MAX)

        val = super().readRegister(_AXP2101_DC_VOL4_CTRL)[0]
        val &= 0xE0
        if millivolt == _AXP2101_DCDC5_VOL_1200MV:
            super().writeRegister(_AXP2101_DC_VOL4_CTRL, val | _AXP2101_DCDC5_VOL_VAL)
        val |= (int)((millivolt - _AXP2101_DCDC5_VOL_MIN) / _AXP2101_DCDC5_VOL_STEPS)
    ...
    def setALDO1Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_ALDO1_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_ALDO1_VOL_STEPS)
        if millivolt < _AXP2101_ALDO1_VOL_MIN:
            raise ValueError("ALDO1 minimum output voltage is  %umV" % _AXP2101_ALDO1_VOL_MIN)
        elif millivolt > _AXP2101_ALDO1_VOL_MAX:
            raise ValueError("ALDO1 maximum output voltage is  %umV" % _AXP2101_ALDO1_VOL_MAX)

        val = super().readRegister(_AXP2101_LDO_VOL0_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_ALDO1_VOL_MIN) / _AXP2101_ALDO1_VOL_STEPS)
    ...
    def setALDO2Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_ALDO2_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_ALDO2_VOL_STEPS)
        if millivolt < _AXP2101_ALDO2_VOL_MIN:
            raise ValueError("ALDO2 minimum output voltage is  %umV" % _AXP2101_ALDO2_VOL_MIN)
        elif millivolt > _AXP2101_ALDO2_VOL_MAX:
            raise ValueError("ALDO2 maximum output voltage is  %umV" % _AXP2101_ALDO2_VOL_MAX)

        val = super().readRegister(_AXP2101_LDO_VOL1_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_ALDO2_VOL_MIN) / _AXP2101_ALDO2_VOL_STEPS)
    ...
    def setALDO3Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_ALDO3_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_ALDO3_VOL_STEPS)
        if millivolt < _AXP2101_ALDO3_VOL_MIN:
            raise ValueError("ALDO3 minimum output voltage is  %umV" % _AXP2101_ALDO3_VOL_MIN)
        elif millivolt > _AXP2101_ALDO3_VOL_MAX:
            raise ValueError("ALDO3 maximum output voltage is  %umV" % _AXP2101_ALDO3_VOL_MAX)

        val = super().readRegister(_AXP2101_LDO_VOL2_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_ALDO3_VOL_MIN) / _AXP2101_ALDO3_VOL_STEPS)
    ...
    def setALDO4Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_ALDO4_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_ALDO4_VOL_STEPS)
        if millivolt < _AXP2101_ALDO4_VOL_MIN:
            raise ValueError("ALDO4 minimum output voltage is  %umV" % _AXP2101_ALDO4_VOL_MIN)
        elif millivolt > _AXP2101_ALDO4_VOL_MAX:
            raise ValueError("ALDO4 maximum output voltage is  %umV" % _AXP2101_ALDO4_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL3_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_ALDO4_VOL_MIN) / _AXP2101_ALDO4_VOL_STEPS)
    ...
    def setBLDO1Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_BLDO1_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_BLDO1_VOL_STEPS)
        if millivolt < _AXP2101_BLDO1_VOL_MIN:
            raise ValueError("BLDO1 minimum output voltage is  %umV" % _AXP2101_BLDO1_VOL_MIN)
        elif millivolt > _AXP2101_BLDO1_VOL_MAX:
            raise ValueError("BLDO1 maximum output voltage is  %umV" % _AXP2101_BLDO1_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL4_CTRL)[0]
        val &= 0xE0
        val |= (int)((millivolt - _AXP2101_BLDO1_VOL_MIN) / _AXP2101_BLDO1_VOL_STEPS)
    ...
    def setBLDO2Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_BLDO2_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_BLDO2_VOL_STEPS)
        if millivolt < _AXP2101_BLDO2_VOL_MIN:
            raise ValueError("BLDO2 minimum output voltage is  %umV" % _AXP2101_BLDO2_VOL_MIN)
        elif millivolt > _AXP2101_BLDO2_VOL_MAX:
            raise ValueError("BLDO2 maximum output voltage is  %umV" % _AXP2101_BLDO2_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL5_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_BLDO2_VOL_MIN) / _AXP2101_BLDO2_VOL_STEPS)
    ...
    def setCPUSLDOVoltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_CPUSLDO_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_CPUSLDO_VOL_STEPS)
        if millivolt < _AXP2101_CPUSLDO_VOL_MIN:
            raise ValueError("CPULDO minimum output voltage is  %umV" % _AXP2101_CPUSLDO_VOL_MIN)
        elif millivolt > _AXP2101_CPUSLDO_VOL_MAX:
            raise ValueError("CPULDO maximum output voltage is  %umV" % _AXP2101_CPUSLDO_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL6_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_CPUSLDO_VOL_MIN) / _AXP2101_CPUSLDO_VOL_STEPS)
    ...
    def setDLDO1Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_DLDO1_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_DLDO1_VOL_STEPS)
        if millivolt < _AXP2101_DLDO1_VOL_MIN:
            raise ValueError("DLDO1 minimum output voltage is  %umV" % _AXP2101_DLDO1_VOL_MIN)
        elif millivolt > _AXP2101_DLDO1_VOL_MAX:
            raise ValueError("DLDO1 maximum output voltage is  %umV" % _AXP2101_DLDO1_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL7_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_DLDO1_VOL_MIN) / _AXP2101_DLDO1_VOL_STEPS)
    ...
    def setDLDO2Voltage(self, millivolt: int) -> None:
        if millivolt % _AXP2101_DLDO2_VOL_STEPS:
            raise ValueError("Step size must be %u mV" % _AXP2101_DLDO2_VOL_STEPS)
        if millivolt < _AXP2101_DLDO2_VOL_MIN:
            raise ValueError("DLDO2 minimum output voltage is  %umV" % _AXP2101_DLDO2_VOL_MIN)
        elif millivolt > _AXP2101_DLDO2_VOL_MAX:
            raise ValueError("DLDO2 maximum output voltage is  %umV" % _AXP2101_DLDO2_VOL_MAX)
        val = super().readRegister(_AXP2101_LDO_VOL8_CTRL)[0] & 0xE0
        val |= (int)((millivolt - _AXP2101_DLDO2_VOL_MIN) / _AXP2101_DLDO2_VOL_STEPS)
    ...
    def setChargingLedMode(self, mode: int) -> None:
        range = [self.XPOWERS_CHG_LED_OFF, self.XPOWERS_CHG_LED_BLINK_1HZ, self.XPOWERS_CHG_LED_BLINK_4HZ, self.XPOWERS_CHG_LED_ON]
        if mode in range:
    ...
    def setChargerConstantCurr(self, opt: int) -> None:
        if not 4 <= opt <= 16:
            raise ValueError("Charger Constant Current must be a value within 4-16!")
    ...
    def setChargeTargetVoltage(self, opt: int) -> None:
        if not 1 <= opt <= 5:
            raise ValueError("Charger target voltage must be a value within 0-3!")
    ...
    def _setInterruptImpl(self, opts: int, enable: bool, debug: bool) -> None:
        if debug:
            ...
            print(" BIN:", end="")
            print(_to_bin(opts, 64))
        if opts & 0x0000FF:
            value = opts & 0xFF
            if debug:
                print("write in ints0 0b{0}".format(_to_bin(value, 8)))
        ...
        if opts & 0x00FF00:
            value = opts >> 8
            if debug:
                print("write in ints1 0b{0}".format(_to_bin(value, 8)))
        ...
        if opts & 0xFF0000:
            value = opts >> 16
            if debug:
                print("write in ints2 0b{0}".format(_to_bin(value, 8)))
    ...
    def printIntRegister(self) -> None:
        for i in range(0, _AXP2101_INTSTS_CNT):
            ...
            print("[{0}]HEX={1} BIN={2}".format(i, hex(val), _to_bin(val, 8)))
    ...
    def compareGaugeData(self, data: bytearray, length: int) -> bool:
        ...
        super().setRegisterBit(_AXP2101_FUEL_GAUGE_CTRL, 0)
        for i in range(128):
            buffer[i] = super().readRegister(_AXP2101_BAT_PARAMS)[0]
        super().clrRegisterBit(_AXP2101_FUEL_GAUGE_CTRL, 0)
    ...
    def deinit(self) -> None:
        ...
        Notes:
            ...
            - 不执行关机操作（仅清除中断配置）
        ==========================================
        Release AXP2101 hardware resources.
        Clears all interrupt enable bits and interrupt status bits.
        Notes:
            - A new instance must be created to use the hardware after this call
            - Does not perform shutdown (only clears interrupt configuration)
        """
        self._log("Deinitializing AXP2101, clearing all interrupts")
        # 禁用所有中断使能位
        super().writeRegister(_AXP2101_INTEN1, 0x00)
        super().writeRegister(_AXP2101_INTEN2, 0x00)
        super().writeRegister(_AXP2101_INTEN3, 0x00)
        # 清除所有中断状态位
        for i in range(_AXP2101_INTSTS_CNT):
            super().writeRegister(_AXP2101_INTSTS1 + i, 0xFF)
            self.statusRegister[i] = 0
            self.intRegister[i] = 0
